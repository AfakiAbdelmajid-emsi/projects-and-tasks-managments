...
def userdetails(Utoken):
    ...
    if user.usertype == 'Admin':
        ...
        # For non-admin users, handle assigned tasks and supervised teams
        ...
        # Query the teams where the user is a member
        teams = Teams.query.join(TeamsMember).filter(TeamsMember.userid == user.userid).all()
        team_ids = [team.team_id for team in teams]
        
        # Query the projects where the user's team is involved
        projects = Project.query.join(ProjectTeam).filter(ProjectTeam.team_id.in_(team_ids)).distinct().all()

        # Query teams where the user is the supervisor
        supervised_teams = Teams.query.filter_by(supervisor_id=user.userid).all()
        
        # Fetch projects related to supervised teams
        supervised_team_ids = [team.team_id for team in supervised_teams]
        supervised_projects = Project.query.join(ProjectTeam).filter(ProjectTeam.team_id.in_(supervised_team_ids)).distinct().all()

        # Combine projects and supervised projects, ensuring no duplicates
        project_ids = {project.project_id for project in projects}  # Use a set to track unique project IDs
...
def update_project(token):
    ...
    all_teams = Teams.query.all()
    project_teams_ids = [pt.team_id for pt in ProjectTeam.query.filter_by(project_id=project.project_id).all()]
    ...
    if request.method == 'POST':
        ...
        if start_date < date.today() and project.start_date != start_date:
            flash('Start date cannot be in the past.', 'danger')
            return render_template('admin/projects/addproject.html', teams=Teams.query.all())
